...
fun2 <- function(x,y){
  #行番号挿入,重複削除,列名置換
  x <- x %>% mutate(No = row_number()) %>% #行番号挿入
    mutate(group = y) %>% #シート番号挿入
    distinct(description,.keep_all=TRUE) %>% #重複削除
    mutate(log10p = -log10(`FDR.value`)) #p値→-log10p値変換
  names(x)[which(names(x)=="X..genes" ) ] <- "Overlap" #列名置換
  #列の作成と結合
  x <- x %>% as.data.frame() %>%
...
dat1_GO_t <- dat1_GO %>% arrange(desc(log10p)) %>% top_n(n = NUM, wt = log10p) %>% #log10pで降順ソートし上位のデータを保存
  mutate(top = if_else(log10p > 0, true = TRUE, false = FALSE)) #topデータをラベル
dat1_BP_t <- dat1_BP %>% arrange(desc(log10p)) %>% top_n(n = NUM, wt = log10p) %>%
  mutate(top = if_else(log10p > 0, true = TRUE, false = FALSE))
...
#svg出力
#GO
svg(file=paste(sprintf("stringApp_GO_top%.0f",NUM),"svg",sep = ".")) #ファイル名指定
print(plotGO) #プロット作成
dev.off() 
#KEGG
svg(file=paste(sprintf("stringApp_KEGG_top%.0f",NUM),"svg",sep = "."))
print(plotKEGG)
dev.off()
#P359###########################################################################
#レシピ14.1 PDFベクタファイルへの出力
#GO
pdf(file=paste(sprintf("stringApp_GO_top%.0f",NUM),"pdf",sep = "."), width = 20/2.54, height = 20/2.54, useDingbats = FALSE) #幅,高さcm(inch/2.54)
print(plotGO)
print(plotGO1)
print(plotGO2)
print(plotGO3)
...
dev.off() #pdf出力
#KEGG
pdf(file=paste(sprintf("stringApp_KEGG_top%.0f",NUM),"pdf",sep = "."), width = 20/2.54, height = 20/2.54, useDingbats = FALSE) #幅,高さcm(inch/2.54)
print(plotKEGG)
print(plotKEGG4)
print(plotKEGG5)
print(plotKEGG6)
...
#レシピ14.1 PDFベクタファイルへの出力
#GO
pdf(file = paste(sprintf("stringApp_volGO_top%.0f",NUM),"pdf",sep = "."), width = 20/2.54, height = 20/2.54, useDingbats = FALSE) #幅,高さcm(inch/2.54)
print(volGO1) #プロット作成
print(volGO2) #プロット作成
print(volGO3) #プロット作成
dev.off() #PDF出力
#KEGG
pdf(file = paste(sprintf("stringApp_volKEGG_top%.0f",NUM),"pdf",sep = "."), width = 20/2.54, height = 20/2.54, useDingbats = FALSE)
print(volKEGG4)
print(volKEGG5)
print(volKEGG6)
dev.off()
...
write.xlsx(smp, file = paste(sprintf("GO_KEGG_top%.0f",NUM),"xlsx",sep = ".")) #GOシート,KEGGシート出力
################################################################################
