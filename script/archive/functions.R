...
plot_set_go <- function(e, showCategory = 20, layout = "nicely", 
  ...
  e <- try(dplyr::mutate(e, qscore = -log(p.adjust, base=10)))
  ## goplot
...
PLA2 <- function(x = read_excel("SWATH.xlsx", 2), 
  ...
  #Duplication table
  data %>% dplyr::group_by(Protein.IDs) %>% dplyr::summarize(frequency = n()) %>% arrange(desc(frequency)) %>% filter(frequency > 1)
  data %>% dplyr::group_by(Gene.names) %>% dplyr::summarize(frequency = n()) %>% arrange(desc(frequency)) %>% filter(frequency > 1)
  data %>% dplyr::group_by(Species) %>% dplyr::summarize(frequency = n()) %>% arrange(desc(frequency)) %>% filter(frequency > 1)
  #Unique Uniprot ID
  ...
  statv <- tdata_rm3 %>% gather(key = GeneName, value = expression, -condition) %>%
    dplyr::group_by(condition, GeneName) %>%
...
## https://github.com/Bioconductor/GenomicFeatures/blob/master/man/makeTxDbFromGRanges.Rd
################################################################################
