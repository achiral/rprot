#stringApp_03_GO_KEGG_top20_cluster_heatmap.R
#5_GO_KEGG_top20に含まれるDEP list作成
#6_GO_KEGG_top20のclusterに含まれるDEP list作成
#7_clusterのDEPのheatmap作成
##############################################################
...
#############################################################
#csv入力
dat1 <- read.csv("PFC_PC10_GO_KEGG_top20_MCL1_node.csv", header=TRUE) #csv入力
dat2 <- read.csv("PFC_PC10_GO_KEGG_top20_MCL2_node.csv", header=TRUE) #csv入力
dat3 <- read.csv("PFC_PC10_GO_KEGG_top20_MCL3_node.csv", header=TRUE) #csv入力
dat4 <- read.csv("PFC_PC10_GO_KEGG_top20_MCL4_node.csv", header=TRUE) #csv入力
dat5 <- read.csv("PFC_PC10_GO_KEGG_top20_MCL5_node.csv", header=TRUE) #csv入力
#xlsx入力
dat6 <- read_excel("heat.xlsx", 1) #シート1入力
#列抽出
t(names(dat1)) #行列の名前確認
#dat1[,14]
dat1 <- dat1 %>% select(14) #列抽出"display.name"
dat2 <- dat2 %>% select(14) #列抽出
dat3 <- dat3 %>% select(14) #列抽出
dat4 <- dat4 %>% select(14) #列抽出
dat5 <- dat5 %>% select(14) #列抽出
...
#xlsx出力
#smp <- list("1"=dat1, "2"=dat2, "3"=dat3,"4"=dat4,"5"=dat5) #リスト作成
#write.xlsx(smp, "list.xlsx") #1-5シート出力
#置換
#dat1$GeneName <- gsub("Eef1b2", "Eef1b", dat1$GeneName) #置換
dat1$GeneName <- gsub("Fh1", "Fh", dat1$GeneName) #置換
dat1$GeneName <- gsub("Gm10221", "Atp5l", dat1$GeneName) #置換
dat3$GeneName <- gsub("Gnb2l1", "Rack1", dat3$GeneName) #置換
dat4$GeneName <- gsub("Epb4.1l1", "Epb41l1", dat4$GeneName) #置換
dat4$GeneName <- gsub("Epb4.1l3", "Epb41l3", dat4$GeneName) #置換
#dat5$GeneName <- gsub("Eno1b", "Eno1", dat5$GeneName) #置換
dat5$GeneName <- gsub("Pgk1-rs7", "Pgk1", dat5$GeneName) #置換
dat5$GeneName <- gsub("Pgm2l1", "TEMPORARY", dat5$GeneName) #置換
dat5$GeneName <- gsub("Pgm2", "Pgm1", dat5$GeneName) #置換
dat5$GeneName <- gsub("TEMPORARY", "Pgm2l1", dat5$GeneName) #置換
...
#############################################################
###   txtを再度入力し、FoldChangeとPvalueのデータを入力   ###
#############################################################
#txt入力
...
#xlsx入力
dat7 <- read_excel("FC_P.xlsx", 1) #シート1入力
#############################################################
#Log2FC値符号変換
...
#############################################################
#txt入力
dat1 <- read.delim("dat1.txt", header=TRUE) #行名入力しない
dat2 <- read.delim("dat2.txt", header=TRUE) #行名入力しない
dat3 <- read.delim("dat3.txt", header=TRUE) #行名入力しない
dat4 <- read.delim("dat4.txt", header=TRUE) #行名入力しない
dat5 <- read.delim("dat5.txt", header=TRUE) #行名入力しない
