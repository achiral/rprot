...
# too many open devices(https://stackoverflow.com/questions/24207960/too-many-open-devices-r)
################################################################################
## 1. instal/load pkg #####
################################################################################
...
ah <- AnnotationHub()
mcols(query(ah, "MeSH.Mmu.eg.db"))
# dbfile1 <- query(ah, c("MeSHDb", "MeSH.Mmu.eg.db", "v002"))[[1]]
dm <- query(ah, "MeSH.Mmu.eg.db")
df <- mcols(dm)
# MeSH.Mmu.eg.db <- query(ah, "MeSH.Mmu.eg.db", "v002")[[1]]
require(clusterProfiler)      #BiocManager::install("clusterProfiler")  # enrichmant analysis
require(DOSE)
require(europepmc)            # install.packages("europepmc")
require(ggnewscale)           # install.packages("ggnewscale")  # enrichment map plot
require(ggplot2)
require(ggupset)              # install.packages("ggupset")
require(Hmisc)                # install.packages("Hmisc")       # llist()
require(MeSHDbi)              # BioC 3.14 (Nov. 2021, with R-4.2.0)
require(meshes)               # BiocManager::install("meshes", force = TRUE)
# require(MeSH.Hsa.eg.db)     # error # BioC 2.14-3.13
require(MeSH.Mmu.eg.db)     # error # BiocManager::install("MeSH.Mmu.eg.db", force = TRUE)  # BioC 2.14-3.13
require(msigdbr)
...
dat_c2 <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_C < 0.05) %>% 
  dplyr::filter(diff_S.C10.S.C0 < 0.05) %>% 
  dplyr::select(ENTREZID, diff_S.C10.S.C0) %>%
  dplyr::arrange(desc(diff_S.C10.S.C0)) %>% 
  t()

dat_c3 <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_C < 0.05) %>% 
  dplyr::filter(diff_S.C30.S.C0 < 0.05) %>% 
  dplyr::select(ENTREZID, diff_S.C30.S.C0) %>%
  dplyr::arrange(desc(diff_S.C30.S.C0)) %>% 
  t()

## PCPxCLZ(interaction)
dat_pc <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_PxC < 0.05) %>% 
  dplyr::select(ENTREZID, diff_P.C0.S.C0) %>%
  dplyr::arrange(desc(diff_P.C0.S.C0)) %>% 
  t()
...
dat_ps <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_PxC < 0.05) %>% 
  dplyr::filter(THSDp_P.C0.S.C0 < 0.05) %>%
  dplyr::select(ENTREZID, diff_P.C0.S.C0) %>%
  dplyr::arrange(desc(diff_P.C0.S.C0)) %>% 
  t()
dat_ps2 <- dat %>%
...
dat_ud <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_PxC < 0.05) %>% 
  dplyr::filter(THSDp_P.C0.S.C0 < 0.05 & diff_P.C0.S.C0 > 0) %>%                                                    # PCP -> up-regulate
  dplyr::filter(THSDp_P.C10.P.C0 < 0.05 & diff_P.C10.P.C0 < 0 | THSDp_P.C30.P.C0 < 0.05 & diff_P.C30.P.C0 < 0) %>%  # CLZ -> down-regulate 
  dplyr::select(ENTREZID, diff_P.C0.S.C0) %>%
  dplyr::arrange(desc(diff_P.C0.S.C0)) %>% 
  t()
dat_ud_df <- dat %>%
...
dat_du <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_PxC < 0.05) %>% 
  dplyr::filter(THSDp_P.C0.S.C0 < 0.05 & diff_P.C0.S.C0 < 0) %>%                                                    # PCP -> down-regulate
  dplyr::filter(THSDp_P.C10.P.C0 < 0.05 & diff_P.C10.P.C0 > 0 | THSDp_P.C30.P.C0 < 0.05 & diff_P.C30.P.C0 > 0) %>%  # CLZ -> up-regulate 
  dplyr::select(ENTREZID, diff_P.C0.S.C0) %>%
  dplyr::arrange(desc(diff_P.C0.S.C0)) %>% 
  t()
dat_du_df <- dat %>%
...
dat_du_df10 <- dat %>%
  ...
  dplyr::filter(q_PxC < 0.05) %>% 
  dplyr::filter(THSDp_P.C0.S.C0 < 0.05 & diff_P.C0.S.C0 < 0) %>%  # PCP -> down-regulate
  dplyr::filter(THSDp_P.C10.P.C0 < 0.05 & diff_P.C10.P.C0 > 0)    # CLZ -> up-regulate 
dat_du_df30 <- dat %>%
  tidyr::drop_na(ENTREZID) %>% 
  dplyr::filter(q_PxC < 0.05) %>% 
  dplyr::filter(THSDp_P.C0.S.C0 < 0.05 & diff_P.C0.S.C0 < 0) %>%  # PCP -> down-regulate
  dplyr::filter(THSDp_P.C30.P.C0 < 0.05 & diff_P.C30.P.C0 > 0)    # CLZ -> up-regulate 
## CLZ attenuate(improve) PCP effect
dat_ef <- rbind(dat_ud_df, dat_du_df) %>% 
  dplyr::select(ENTREZID, diff_P.C0.S.C0) %>%
  dplyr::arrange(desc(diff_P.C0.S.C0)) %>% 
  t()
dat_ef_df <- rbind(dat_ud_df, dat_du_df)
...
geneList_ef <- prepare_geneList(dat_ef)


GENELIST <- list(geneList_p, geneList_c, geneList_pc, geneList_ps,
                 geneList_ud, geneList_du, geneList_uu, geneList_dd, geneList_ef)
...
gene_ef <- names(geneList_ef)
GENE <- Hmisc::llist(gene_p, gene_c, gene_pc, gene_ps,
                     gene_ud, gene_du, gene_uu, gene_dd, gene_ef)
################################################################################
## set parameters #####
################################################################################
tic()
...
toc()
################################################################################
## $$$$$ expression data, output directory $$$$$ #####
################################################################################
## move to host working directory
setwd(DIR_HOST)

## set multiple gene list (13. Comparing multiple gene list) #####
MULTI_GENE <- Hmisc::llist(gene_p, gene_c, gene_pc)
MULTI_GENE <- Hmisc::llist(gene_ps, gene_ud, gene_du, gene_uu, gene_dd, gene_ef)

## set gene data, output directory #####
## p
DAT_GENE <- dat_p
DIR_OUT <- "p"
SET_DIR_OUT <- "./p"
...
# ENRICH_SET(DAT_GENE)
################################################################################
## ENRICH_SET function # error(cannot output images -> clear devices -> error) #####
# ENRICH_SET <- function(DAT_GENE     = DAT_GENE,                                 # gene list
#                        DIR_OUT      = DIR_OUT,                                  # output directory
#                        SET_DIR_OUT  = SET_DIR_OUT,                              # move to output directory
#                        MULTI_GENE   = MULTI_GENE,                               # multiple gene set
#                        ont          = "BP",                                     # "BP"(default ),"CC","MF","ALL"
#                        showCategory = 20,                                       # default = 20
#                        layout       = "kk",                                     # "kk", "nicely"
#                        labeln       = "all",                                    # "category", "gene", "all", "none"
#                        db           = org.Mm.eg.db,                             # database
#                        d            = GOSemSim::godata(OrgDb = db, ont = ont),  # emap plot
#                        MeSHDb       = "MeSH.Mmu.eg.db"){}                        # MeSH.Hsa.eg.db    # BioC 2.14-3.13)
################################################################################################################################################################
## $$$$$ start program $$$$$ #####
...
################################################################################
## 13. Comparing multiple gene list #####
# MULTI_GENE <- Hmisc::llist(gene_p, gene_c, gene_pc)
## compare KEGG pathway
ck <- compareCluster(geneCluster = MULTI_GENE, 
...
################################################################################
## 14. Formula interface of compareCluster #####
################################################################################
## 14. functional profile comparison #####
...
fr <- setReadable(fr, OrgDb = db, keyType="ENTREZID")
################################################################################
################################################################################
################################################################################
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}      # create output directory
setwd(SET_DIR_OUT)                                 # move to output directory
if(!dir.exists("05_go")){dir.create("05_go")}      # create output directory
setwd("./05_go")                                   # move to output directory
...
plot_set_go(ego, 
            ...
            filedp2 = "dp2_ego.svg",                 # filedp2 = "dp2.svg"
...
plot_set_go(egosim, 
            ...
            filedp2 = "dp2_egosim.svg",              # filedp2 = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}      # create output directory
setwd(SET_DIR_OUT)                                 # move to output directory
if(!dir.exists("06_kegg")){dir.create("06_kegg")}  # create output directory
setwd("./06_kegg")                                 # move to output directory
...
plot_set(ek, 
         ...
         filedp2      = "dp2_ek.svg",           # filedp2  = "dp2.svg"
...
################################################################################
## 7. WikiPathways over-representation analysis #####
################################################################################
## file save directory #####
## make directory to save images of enrichment analysis
setwd(DIR_HOST)                                    # move to host working directory
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}      # create output directory
setwd(SET_DIR_OUT)                                 # move to output directory
if(!dir.exists("07_wiki")){dir.create("07_wiki")}  # create output directory
setwd("./07_wiki")                                 # move to output directory
...
plot_set(ewp2, 
         ...
         filedp2      = "dp2_ewp.svg",           # filedp2 = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}              # create output directory
setwd(SET_DIR_OUT)                                         # move to output directory
if(!dir.exists("08_reactome")){dir.create("08_reactome")}  # create output directory
setwd("./08_reactome")                                     # move to output directory
...
plot_set(er, 
         ...
         filedp2      = "dp2_er.svg",           # filedp2 = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}            # create output directory
setwd(SET_DIR_OUT)                                       # move to output directory
if(!dir.exists("09_disease")){dir.create("09_disease")}  # create output directory
setwd("./09_disease")                                    # move to output directory
...
#          filedp2      = "dp2_emesh.svg",           # filedp2 = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}      # create output directory
setwd(SET_DIR_OUT)                                 # move to output directory
if(!dir.exists("10_mesh")){dir.create("10_mesh")}  # create output directory
setwd("./10_mesh")                                 # move to output directory
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}                  # create output directory
setwd(SET_DIR_OUT)                                             # move to output directory
if(!dir.exists("11_cellmarker")){dir.create("11_cellmarker")}  # create output directory
setwd("./11_cellmarker")                                       # move to output directory
...
plot_set(ecm2, 
         ...
         filedp2      = "dp2_ecm.svg",           # filedp2 = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}          # create output directory
setwd(SET_DIR_OUT)                                     # move to output directory
if(!dir.exists("12_msigdb")){dir.create("12_msigdb")}  # create output directory
setwd("./12_msigdb")                                   # move to output directory
...
plot_set(emsig_H_2, 
         ...
         filedp2      = "dp2_emsig_H.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C1_2, 
         ...
         filedp2      = "dp2_emsig_C1.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C2_2, 
         ...
         filedp2      = "dp2_emsig_C2.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C3_2, 
         ...
         filedp2      = "dp2_emsig_C3.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C4_2, 
         ...
         filedp2      = "dp2_emsig_C4.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C5_2, 
         ...
         filedp2      = "dp2_emsig_C5.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C6_2, 
         ...
         filedp2      = "dp2_emsig_C6.svg",           # filedp2  = "dp2.svg"
...
plot_set(emsig_C7_2, 
         ...
         filedp2      = "dp2_emsig_C7.svg",           # filedp2  = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}                # create output directory
setwd(SET_DIR_OUT)                                           # move to output directory
if(!dir.exists("13_multilist")){dir.create("13_multilist")}  # create output directory
setwd("./13_multilist")                                      # move to output directory
...
plot_set(ck, 
         ...
         filedp2      = "dp2_ck.svg",           # filedp2  = "dp2.svg"
...
plot_set(cr, 
         ...
         filedp2      = "dp2_cr.svg",           # filedp2  = "dp2.svg"
...
if(!dir.exists(DIR_OUT)){dir.create(DIR_OUT)}                            # create output directory
setwd(SET_DIR_OUT)                                                       # move to output directory
if(!dir.exists("14_functionprofile")){dir.create("14_functionprofile")}  # create output directory
setwd("./14_functionprofile")                                            # move to output directory
