...
dat1 <- dat1 %>% as.data.frame() %>%
  mutate(DBsourse = case_when(                  #DBsourse列作成
    ...
    `category` == "GO Function" ~ "MP",     #categoryのGO FunctionをMP
...
#############################################################
#top50データをラベル
dat1_f <- mutate(dat1_f, top50 = if_else(log10p > 0, true = TRUE, false = FALSE))
dat2_f <- mutate(dat2_f, top50 = if_else(log10p > 0, true = TRUE, false = FALSE))
dat3_f <- mutate(dat3_f, top50 = if_else(log10p > 0, true = TRUE, false = FALSE))
dat4_f <- mutate(dat4_f, top50 = if_else(log10p > 0, true = TRUE, false = FALSE))
dat5_f <- mutate(dat5_f, top50 = if_else(log10p > 0, true = TRUE, false = FALSE))
dat6_f <- mutate(dat6_f, top50 = if_else(log10p > 0, true = TRUE, false = FALSE))
#############################################################
