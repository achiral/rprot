...
detach_all <- function() {
  basic.pkg <- c("package:stats", "package:graphics", "package:grDevices", 
                 ...
                 "package:base")
  pkg.list <- search()[ifelse(unlist(gregexpr("package:", 
                                              search())) == 1 ,TRUE, FALSE)]
...
#フルパスの確認
dir.choose <- function() {
  system("osascript -e 'tell app \"RStudio\" to POSIX path of 
         (choose folder with prompt \"Choose Folder:\")' > /tmp/R_folder",
...
ls1 <- grep("60 kDa .eat shock protein|Actin|actin|.apdh|Histon|.inculin|
            Lamin.B1|Serotransferrin|Tubulin|tubulin|Vdac1|.yclophilin|
            .ytochrome",anno$Group)
ls2 <- grep("P68134|P68368|P05213|P60710|Q7TMM9|Q9ERD7|Q9D6F9|P99024|TUBB1|
            P18760|P19783|P16858|O09106|P68433|P63038|P14733|P14094|Q8VDN2|
            Q9WV27|P17918|P29037|Q60932|Q64727|Q00899",anno$Protein.IDs)
anno2 <- anno[ls1,]
anno3 <- anno[ls2,]
ls_rm1 <- c(grep("acting|binding|.ctinin|cuppling|deacetylase|folding|
                 F.actin|.like|H2|H4|lysine|macro|NADPH|ractin|regulator|
                 tactin|polymer|.related|.sociated|specific|subunit|tactin|
                 .ytochrome.b|linking",
...
mean_table <- function(dat_mean){
  ...
  ACTBxG3P <- x2[grep("P60710|P16858",dat_mean$Protein.IDs),] #ACTB,G3P(GAPDH)でfilter
  ...
  x3 <- rbind(x2,ACTBxG3P_m,ACTBxG3P_gm)
  #return(invisible(x3))
} #LC_prc_meanフォーマットの全サンプル算術平均とACTBxG3Pの算術・幾何平均を追加
opt <- mean_table(LC_prc_mean) #output
LC_prc_mean <- opt #LC_prc_meanに戻す
dat_prc_value <- dat_prc[,grep("GeneName|_1$|_2$|_3$|_4$|_5$|_6$|
                               _7$|_8$|_9$|_10$|_11$|_12$",colnames(dat_prc))] #数値データ抽出
