...
################################################################################
################################################################################
dat_prc_value <- dat_prc[,grep("GeneName|_1$|_2$|_3$|_4$|_5$|_6$|_7$|_8$|_9$|_10$|_11$|_12$",colnames(dat_prc))]
...
for(k in 1:nrow(LC_prc_mean)){
  ...
  #############################################################
  #output xlsx
  ls_stat[[k]] <- as.data.frame(sdata3)
  #write_xlsx(ls_stat, "stat_prc.xlsx", format_headers = FALSE)
  #############################################################
  #DEP list
  twANOVA_Pq005 <- sdata3 %>% filter(Species == "MOUSE") %>% filter(q_P < 0.05)
  twANOVA_Cq005 <- sdata3 %>% filter(Species == "MOUSE") %>% filter(q_C < 0.05)
  twANOVA_PxCq005 <- sdata3 %>% filter(Species == "MOUSE") %>% filter(q_PxC < 0.05)
  ls_DEPtwANOVA_Pq005[[k]] <- as.data.frame(twANOVA_Pq005[,grep("(GeneName|p_P$|p_C$|p_PxC$|q_P$|q_C$|q_PxC$|Protein.IDs|Description|GN)", colnames(twANOVA_Pq005))])
  ls_DEPtwANOVA_Cq005[[k]] <- as.data.frame(twANOVA_Cq005[,grep("(GeneName|p_P$|p_C$|p_PxC$|q_P$|q_C$|q_PxC$|Protein.IDs|Description|GN)", colnames(twANOVA_Cq005))])
  ls_DEPtwANOVA_PxCq005[[k]] <- as.data.frame(twANOVA_PxCq005[,grep("(GeneName|p_P$|p_C$|p_PxC$|q_P$|q_C$|q_PxC$|Protein.IDs|Description|GN)", colnames(twANOVA_PxCq005))])
}
