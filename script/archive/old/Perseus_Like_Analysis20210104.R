...
#multcomp　　　https://astatsa.com/OneWay_Anova_with_TukeyHSD/_Rcode_tutorial/
#エクセル入力　http://www.yujitakenoshita.org/post/read-excel-in-r/
#列削除　　　　http://byungdugjun.blogspot.com/2014/07/r-x-image-disease-1-1-1-2-2-0-3-0-1-4-1.html
#BH法　　　　　https://stats.biopapyrus.jp/stats/fdr-bh.html
#THSD　　　　　https://qiita.com/hfu62/items/f9f4803828fd7e1a5cec
#My anova loop prints out the same results in R　　https://stackoverflow.com/questions/50914023/my-anova-loop-prints-out-the-same-results-in-r
#############################################################
#############################################################
#############################################################
#log2-Impute(MNAR)-Subtract(Median):like a Perseus
...
#statistic summary
#############################################################
#install.packages("summarytools") #takes a long time
#library(summarytools)
#statv <- stby(data = tdata_rm3,
#     INDICES = tdata_rm3$condition,
#     FUN = descr,
#     #stats = c("mean", "sd", "min", "Q1", "med", "Q3", "max"),
#     transpose = T)
#dataframe_SC0 <- as.data.frame(statv[["SC0"]]) #dataframe
#############################################################
##########################################################################################################################
#############################################################
#statv <- tdata_rm3 %>% gather(key = GeneName, value = expression, -condition) %>%
#  group_by(condition, GeneName) %>% 
#  summarise_each(list(N = length, min = min, max = max, med = median, mean = mean, sd = sd, IQR = IQR))
statv <- tdata_rm3 %>% gather(key = GeneName, value = expression, -condition) %>%
  group_by(condition, GeneName) %>%
